    ...
    <div className="pt-24 pb-12 min-h-screen bg-secondary/10 flex flex-col items-center px-4">
      <div className="max-w-md w-full space-y-8">
        <div className="text-center space-y-2">
          ...
          <p className="text-muted-foreground italic">Official identity document for GGLMSS Platform</p>
        </div>
        ...
        <div id="id-card-capture" className="relative w-full aspect-[1.6/1] bg-white rounded-3xl overflow-hidden shadow-2xl border-4 border-white">
          ...
          <div className="absolute top-0 left-0 right-0 h-24 bg-primary flex items-center px-6 gap-4">
            <div className="w-12 h-12 relative bg-white rounded-full p-1.5 shrink-0">
            ...
            <div>
              ...
              <p className="text-accent text-[8px] font-bold uppercase tracking-widest">{student.motto}</p>
            </div>
          </div>
          ...
          <div className="mt-24 p-6 flex gap-6">
            ...
            <div className="flex-1 space-y-3 py-2">
              <div>
                <p className="text-[8px] font-bold text-muted-foreground uppercase">Full Name</p>
                <p className="text-lg font-black text-primary leading-none uppercase">{student.name}</p>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <p className="text-[8px] font-bold text-muted-foreground uppercase">Class</p>
                  <p className="text-sm font-bold text-primary">{student.class} - {student.section}</p>
                </div>
                <div>
                  <p className="text-[8px] font-bold text-muted-foreground uppercase">Roll No</p>
                  <p className="text-sm font-bold text-primary">{student.roll}</p>
                </div>
              </div>
              <div>
                <p className="text-[8px] font-bold text-muted-foreground uppercase">Student ID</p>
                <p className="text-sm font-bold text-accent font-mono">{student.id}</p>
              </div>
            </div>
          </div>
          ...
          <div className="absolute bottom-4 left-6 flex items-center gap-2">
             <Badge className="bg-emerald-500 text-[8px] uppercase px-2 py-0.5">Verified</Badge>
             <span className="text-[8px] text-muted-foreground font-bold italic font-mono tracking-widest uppercase">Valid Session 2024-25</span>
          </div>
        </div>

        <Button className="w-full h-12 rounded-xl gap-2 shadow-xl text-lg" onClick={() => window.print()}>
